...
public class Ogre extends Entity implements Serializable{
	...
	private void moveMassiveClub( Board board, char heroIcon, Point keyPosition){
		...
		if( board.symbolAt(keyPosition.getX(),keyPosition.getY() )==' ' && heroIcon =='A')
	...
	public void move(Board board, String direction, Point keyPosition, char heroIcon){
		if( roundsStunnedCounter==0){
			...
			Point absolutePoint= new Point(pos.getX()+relativepoint.getX(), pos.getY()+relativepoint.getY());
			if(absolutePoint.getX() == keyPosition.getX() && absolutePoint.getY() == keyPosition.getY() && heroIcon!='K'){
				ogreIcon='$';
			}
			if(pos.getX()==keyPosition.getX() && pos.getY() == keyPosition.getY()){
				if(heroIcon=='A')	board.placeSymbol(pos.getX(), pos.getY(), 'k');
				else board.placeSymbol(pos.getX(), pos.getY(), ' ');
				ogreIcon='O';}
			else {
